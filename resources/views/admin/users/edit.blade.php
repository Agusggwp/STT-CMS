...
@section('title', 'Edit Pengguna - Admin CMS')
...
<div class="max-w-3xl space-y-6">
    <div class="pb-6 border-b border-[#E5E5E5]">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-[#1F1F1F]">Edit Data Pengguna</h1>
        <p class="text-sm text-[#6B6B6B] mt-1">Perbarui profil, peranan, atau status akun {{ $user->name }}.</p>
    </div>
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-6 bg-white border border-[#E5E5E5] space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-xs font-bold text-[#1F1F1F] uppercase tracking-wider mb-2">Password Baru (Kosongkan jika tidak diganti)</label>
                <input type="password" name="password" placeholder="Biarkan kosong jika tidak diubah" class="w-full px-4 py-2.5 text-sm bg-white border border-[#E5E5E5] focus:outline-none focus:border-[#8B1E24]">
            </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-6 border-t border-[#E5E5E5]">
            ...
            <button type="submit" class="px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white bg-[#8B1E24] hover:bg-[#6e171c]">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
