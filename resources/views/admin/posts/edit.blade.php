...
@section('page_title', 'Ubah Berita: ' . Str::limit($post->title, 40))
        ...
        <div class="p-6 sm:p-8 border border-[#E5E5E5] bg-white space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 border border-[#E5E5E5] bg-white">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-[#1F1F1F] mb-1.5">
                        Unggah Gambar Thumbnail Baru
                    </label>
                    @if($post->thumbnail)
                        <div class="mb-2">
                            <img src="{{ $post->thumbnail_url }}" alt="Preview" class="w-24 h-16 object-cover border border-[#E5E5E5]">
                        </div>
                    @endif
                    <input type="file"
                           name="thumbnail"
                ...
                <div>
                    ...
                    <input type="url"
                           name="thumbnail_url"
                           value="{{ old('thumbnail_url', str_starts_with($post->thumbnail ?? '', 'http') ? $post->thumbnail : '') }}"
                           placeholder="https://..."
            ...
            <div>
                ...
                <textarea name="summary"
                          id="summary"
                          rows="2"
                          class="w-full px-3.5 py-2 bg-white border border-[#E5E5E5] text-sm text-[#1F1F1F] focus:border-[#8B1E24] focus:outline-hidden">{{ old('summary', $post->summary) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="content"
                          ...
                          rows="10"
                          required
                          class="w-full px-3.5 py-2.5 bg-white border border-[#E5E5E5] text-sm text-[#1F1F1F] font-mono focus:border-[#8B1E24] focus:outline-hidden">{{ old('content', $post->content) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4 border-t border-[#E5E5E5]">
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-3 py-2 bg-white border border-[#E5E5E5] text-xs font-semibold focus:border-[#8B1E24] focus:outline-hidden">
                        <option value="published" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>Published (Tayang)</option>
                ...
                <div class="flex items-center pt-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="accent-[#8B1E24]">
                        <span class="text-xs font-bold text-[#1F1F1F]">Jadikan Berita Utama</span>
                    </label>
                </div>
            </div>
            ...
            <div class="p-4 border border-[#E5E5E5] bg-white space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="w-full px-3 py-1.5 bg-white border border-[#E5E5E5] text-xs">
                </div>
                <div>
                    <label class="block text-[11px] font-semibold text-[#1F1F1F] mb-1">Meta Description</label>
                    <textarea name="meta_description" rows="2" class="w-full px-3 py-1.5 bg-white border border-[#E5E5E5] text-xs">{{ old('meta_description', $post->meta_description) }}</textarea>
                </div>
            </div>
